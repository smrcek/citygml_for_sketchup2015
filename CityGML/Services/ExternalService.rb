...
module FHGelsenkirchen
  ...
  class FeedbackService
    def initialize
      ...
      result = ""
      result << %Q["#{str}"] +' '+ FHGelsenkirchen::CITYGML_VERSION.to_s + ' ' + FHGelsenkirchen::CITYGML_BUILD.to_s
      IO.popen(result)
    end
  end
  ...
  class UpdateService
    def initialize
      ...
      str = str.encode("WINDOWS-1252")
      result = ""
      result << %Q["#{str}"] +' '+ 'Editor '+ FHGelsenkirchen::CITYGML_VERSION.to_s+' '+ FHGelsenkirchen::CITYGML_BUILD.to_s
      IO.popen(result)
    end
  end
